...
//		See the file kd_Leaf_search.cpp for general information on the
//		approximate nearest neighbor search algorithm.  Here we
//		include the extensions for shrinking nodes.
//----------------------------------------------------------------------

//----------------------------------------------------------------------
//	bd_shrink::ann_Leaf_search - search a shrinking node
//----------------------------------------------------------------------

void ANNbd_shrink::ann_Leaf_search(ANNdist box_dist)
{
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_Leaf_search(inner_dist);// search inner child first
		child[ANN_OUT]->ann_Leaf_search(box_dist);// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_Leaf_search(box_dist);// search outer child first
		child[ANN_IN]->ann_Leaf_search(inner_dist);// ...then outer child
	}
